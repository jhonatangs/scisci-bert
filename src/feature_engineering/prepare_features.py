...
def build_feature_set(processed_data_path: str, output_path: str):
    ...
    try:
        ...
        print(
            f"Erro: Arquivo de dados processados não encontrado em '{processed_data_path}'"
        )
        return

    documents = []
    for _, row in tqdm(
        enriched_df.iterrows(), total=enriched_df.shape[0], desc="Preparando documentos"
    ):
    ...
    if not os.path.exists(processed_parquet):
        ...
        data = {
            ...
            "x_concepts": [
                ...
                [
                    {"display_name": "Computer Networks", "score": 0.9},
                    {"display_name": "Security", "score": 0.6},
                ],
            ],
        }
